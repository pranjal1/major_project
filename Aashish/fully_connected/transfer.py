...
def create_inception_graph():
  ...
  with tf.Graph().as_default() as graph:
    ...
    with gfile.FastGFile(model_filename, 'rb') as f:
      ...
      #Debug Codes
      print(tf.shape(bottleneck_tensor))
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, image_dir, category, bottleneck_dir, jpeg_data_tensor, bottleneck_tensor):
  ...
  if did_hit_error:
    ...
    #Debug Codes
    bottleneck_values_2 = get_bottleneck(image_lists, label_name, index,
                                        bottleneck_dir, category)
    if (bottleneck_values == bottleneck_values_2):
      print("Default Bottleneck values is same as that read from file! SUCCESS!!!")
    else:
      print("Bottleneck values mismatch! :(")
...
#Debug Codes
def get_bottleneck(image_lists, label_name, index, bottleneck_dir, category):
  bottleneck_path = get_bottleneck_path(image_lists, label_name, index,
                                        bottleneck_dir, category)
  with open(bottleneck_path, 'r') as bottleneck_file:
    bottleneck_string = bottleneck_file.read()
  ...
  with tf.Session(graph=graph) as sess:
    ...
    #print(image_lists)
      ...
      cache_bottlenecks(sess, image_lists, FLAGS.image_dir,
                        ...
                        bottleneck_tensor)
      #np.savez(os.path.join(FLAGS.bottleneck_dir,"info.npz"), image_lists=image_lists)
      with open(os.path.join(FLAGS.bottleneck_dir,"info.pkl"), 'wb') as f:
        pickle.dump(image_lists, f, pickle.HIGHEST_PROTOCOL)
    ...
    print(np.squeeze(np.array(train_bottlenecks)).shape)
...
def parseFunction():
  ...
  parser.add_argument(
      '--output_labels',
      type=str,
      default='output_graph/output_labels.txt',
  ...
  parser.add_argument(
      '--model_dir',
      type=str,
      default='/home/aashish/Desktop/InceptionModel/',
      help="""\
